...
class AutoProcessLifecycleTrackerTest : KotlinBaseTest() {
    ...
    private fun createPlugin(): MeasurementPlugin = AutoAppLifecycleTracker(
    ...
    @Test
    fun `lifecycle events cause IOLEvent emissions`() {
        val plugin = createPlugin()
        ...
        eventSub.awaitCount(1).assertNotComplete().assertValue {
            it is Event.Tracking && it.iolEvent.state == "start"
        }

        lifecycleRegistry.currentState = Lifecycle.State.STARTED
        testScheduler.triggerActions()
        eventSub.awaitCount(2).assertNotComplete().assertValueAt(1) {
            it is Event.Tracking && it.iolEvent.state == "enterForeground"
        }

        lifecycleRegistry.currentState = Lifecycle.State.DESTROYED
        testScheduler.triggerActions()
        eventSub.awaitCount(3).assertNotComplete().assertValueAt(2) {
            it is Event.Tracking && it.iolEvent.state == "enterBackground"
        }
