...
import de.infonline.lib.iomb.events.internal.IOLApplicationEventPrivate.IOLApplicationEventPrivateType
...
@PerMeasurement
internal class AutoAppLifecycleTracker @Inject constructor(
    scheduler: Scheduler,
    ...
    private val lifecycleMonitor = object : LifecycleObserver {
        ...
        fun onAppCreated() {
            IOLLog.tag(TAG).d("Lifecycle event: App Start.")
            publisher.onNext(
                Event.Tracking(
                    iolEvent = IOLApplicationEventPrivate(IOLApplicationEventPrivateType.Start),
                    isAutoEvent = true
                )
            )
        }
        ...
        fun onMoveToForeground() {
            IOLLog.tag(TAG).d("Lifecycle event: App EnterForeground.")
            publisher.onNext(
                Event.Tracking(
                    iolEvent = IOLApplicationEventPrivate(IOLApplicationEventPrivateType.EnterForeground),
                    isAutoEvent = true
                )
            )
        }
        ...
        fun onMoveToBackground() {
            ...
            publisher.onNext(
                Event.Tracking(
                    iolEvent = IOLApplicationEventPrivate(IOLApplicationEventPrivateType.EnterBackground),
                    isAutoEvent = true
                )
            )
        }
    ...
    companion object {
        const val TAG = "AutoAppLifecycleTracker"
    }
