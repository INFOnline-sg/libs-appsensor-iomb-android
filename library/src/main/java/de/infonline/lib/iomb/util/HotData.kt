...
internal open class HotData<T>(
    ...
    init {
        initialValue
                ...
                .observeOn(scheduler)
                .doOnError { IOLLog.tag(TAG).e(it, "Error while providing initial value.") }
                .subscribe(
        ...
        updatePub
                ...
                .concatMap { action ->
                    statePub.take(1).map { oldState ->
                        ...
                        if (debugOutput) IOLLog.tag(TAG).v("Update ${oldState.data} -> $newData")
                        if (newData != null) {
                            State(
                                    data = newData,
                                    actionId = action.id
                ...
                .doOnError { IOLLog.tag(TAG).e(it, "Error while updating value.") }
                .subscribe(
                        {
